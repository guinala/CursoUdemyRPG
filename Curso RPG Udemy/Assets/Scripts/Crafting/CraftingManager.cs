...
public class CraftingManager : Singleton<CraftingManager>
{
    ...
    public void Display(Receipe receipe)
    {
        ...
        amountMaterial1.text = $"{Inventory.Instance.ObtainItemsAmount(receipe.Material1.id)} / {receipe.amountRequired1}";
        amountMaterial2.text = $"{Inventory.Instance.ObtainItemsAmount(receipe.Material2.id)} / {receipe.amountRequired2}";
    ...
    {
        if(Inventory.Instance.ObtainItemsAmount(receipe.Material1.id) >= receipe.amountRequired1 &&
                       Inventory.Instance.ObtainItemsAmount(receipe.Material2.id) >= receipe.amountRequired2)
        {
    ...
    public void Craft()
    {
        ...
        {
            Inventory.Instance.ConsumeItem(selectedReceipe.Material1.id);
        }

        for(int i = 0; i < selectedReceipe.amountRequired2; i++)
        {
            Inventory.Instance.ConsumeItem(selectedReceipe.Material2.id);
        }

        Inventory.Instance.AddItem(selectedReceipe.Result, selectedReceipe.amountResult);
